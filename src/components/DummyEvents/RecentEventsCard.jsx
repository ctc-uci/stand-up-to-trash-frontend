    ...
    <Box maxWidth="434px" width="100%" height="280px">
      <Box my="13px" mx="25px">
        ...
        <Box mt="12px" display="flex" flexDir="row" gap="14px">
          <Card width='411px' height='240px' flexShrink='0' backgroundColor="#FFEBEF8F" display="flex" flexDir="column" justifyContent='center' alignItems='center' borderRadius='30px' boxShadow='0px 4px 4px 0px rgba(0, 0, 0, 0.25)'>
            <Heading fontSize='20px'><u>{events.length > 0 ? events[eventIndex].name : ''}</u></Heading>
            <Box display="flex" flexDir="row" alignItems='center' justifyContent='center'>
              <Button variant="ghost" width='20px' onClick={handlePrevious}><ChevronLeftIcon/></Button>
              <Box w="170px" h="170px">
                <Box
                  mx="13px"
                  display="flex"
              ...
              <Button variant="ghost" width='20px' _hover={{ color: "None" }} onClick={handleNext}><ChevronRightIcon/></Button>
            </Box>
          </Card>
        </Box>
      </Box>
