...
const AddEventsModal = () => {
    ...
    <Modal isOpen={isOpen} onClose={handleCancel}>
        <ModalOverlay />
        <ModalContent minW={"40%"} borderRadius={'30px'} boxShadow={'0px 6.9760003089904785px 6.9760003089904785px 0px #00000080'}>
            <ModalHeader>
                <ModalHeader fontSize={'24px'} justify={'center'} 
                align={'center'} fontFamily={'Inter'}
                fontWeight={'700'} lineHeight={'29.05px'} marginBottom={'-25px'}
            ...
            <ModalBody>
                <FormControl>
                   ...
                   { isLoading ?  <Spinner/> :
                    <Dropzone setEventData={setEventData} eventData={eventData} height={'141px'} setIsLoading={setIsLoading}/>}
                    </Flex>

                    <FormLabel paddingTop={'10px'} fontFamily={'Inter'} fontWeight={'700'} fontSize={'12px'}>Event Name</FormLabel>
                        <Input type='text' placeholder='Beach Cleanup' onChange={(e) => {setEventData({...eventData, name: e.target.value})}}/>
                        <Flex flexDirection={'row'}>
                            <Flex flexDirection={'column'} width={'100%'} paddingRight={'10px'}>
                                <FormLabel paddingTop={'10px'} fontFamily={'Inter'} fontWeight={'700'} fontSize={'12px'}>Date</FormLabel>
                                <Input type='date' onChange={(e) => {
                                    setEventData({...eventData, date: e.target.value})}}/>
                            </Flex>
                            <Flex flexDirection={'column'} width={'100%'} paddingLeft={'10px'}>
                                <FormLabel paddingTop={'10px'} fontFamily={'Inter'} fontWeight={'700'} fontSize={'12px'}>Time</FormLabel>
                                <Input type='time' onChange={(e) => {
                                    setEventData({...eventData, time: e.target.value})}}/>
                            </Flex>
                        </Flex>
                    <FormLabel paddingTop={'10px'} fontFamily={'Inter'} fontWeight={'700'} fontSize={'12px'}>Location</FormLabel>
                        <Input type='text' placeholder='Dana Point' onChange={(e) => {setEventData({...eventData, location: e.target.value})}}/>
                    <FormLabel paddingTop={'10px'} fontFamily={'Inter'} fontWeight={'700'} fontSize={'12px'}>Add Event Description</FormLabel>
                        <Textarea type='text' placeholder='Event Description, etc...' height={'109px'} onChange={(e) => {setEventData({...eventData, description: e.target.value})}}/>
                    <FormLabel paddingTop={'10px'} fontFamily={'Inter'} fontWeight={'700'} fontSize={'12px'}>Add Waivers</FormLabel>
                        <IconButton icon={<AttachmentIcon/>} borderRadius={'30px'}/>
                </FormControl>
            </ModalBody>

            <ModalFooter justifyContent={'center'}>
                <Button leftIcon={<CancelIcon/>} onClick={handleCancel} backgroundColor={'#FFFFFF'} borderRadius={'30px'} marginRight={'10px'}>Cancel</Button>
                <Button leftIcon={<CreateEventIcon/>} backgroundColor={'#95D497'} borderRadius={'30px'} marginLeft={'10px'} onClick={handleSubmit} isDisabled={isSubmittable}>Create Event</Button>
            </ModalFooter>
        </ModalContent>
    </Modal>
    </ChakraProvider>)
